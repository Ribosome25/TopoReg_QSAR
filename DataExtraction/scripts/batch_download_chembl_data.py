...
targets = pd.read_excel("~/Topo_Regression/topological_regression/data/ChEMBL/Large_scale SM2.xlsx", sheet_name=1, index_col=0).index
...
for ID in tqdm(targets):
    ...
    n_isomers = []
    mol_weights = []
    for idx, item in df_to.iterrows():  # 立体异构体数
        sml = item["Smiles"]
        if sml is None:
    ...
    df_to.to_csv("~/Topo_Regression/topological_regression/data/ChEMBL/{}/{}/py_downloaded.tsv".format(batch_tag, ID), sep="\t")
